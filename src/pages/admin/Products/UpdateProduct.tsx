...
import { schemaUpdateProduct } from '../../../validation/product'
...
export default function UpdateProduct() {
  ...
  useEffect(() => {
    ...
    setCaregoryId(product.data.category.id)
  ...
  }, [getProduct?.data?.data, product_id])
  ...
  // --- Set Brand Id --- //
  useEffect(() => {
    const product = getProduct.data?.data as GetProductResponseSuccess | undefined
    if (product && getBrandByCategoryId.data?.data?.data?.brands?.length > 0) {
      setValue('brand_id', product.data.brand.id)
    }
  }, [getBrandByCategoryId.data, getProduct.data])
  ...
  const updateProductMutation = useMutation({
    mutationFn: (body: {
      ...
      adminProductApi.updateProduct(product_id, {
        ...body,
  ...
  const handleSubmitUpadateProduct = handleSubmit((data: any) => {
  ...
  return (
    <div className='h-full'>
      ...
      <form
        ...
        method='post'
        onSubmit={handleSubmitUpadateProduct}
      >
        {/* Row 1 */}
        <Input
          label='Tên Sản Phẩm'
            ...
            render={({ field }) => (
              <div className='rounded-lg overflow-hidden border border-solid border-[#B3B3B3]'>
                <ReactQuill
                  ...
                  className='block text-[15px] leading-[1.5] bg-transparent outline-none resize-none max-h-[300px] overflow-y-auto'
                />
              </div>
            )}
          />
